...
--Current entity: segler
--New Entity: palm beach

SELECT  [ClientId] ,[Name], [Address],[City],[StateCd]
  FROM [Clients]  where ClientId=30
  where Name in ( 'Segler AP c/o Miller Grossbard Advisors LLP','REME LLC')
...
select @beginlocationid = 400, @endlocationid = 435
...
(	[NewLocationId] [bigint] IDENTITY(400,1) NOT NULL,				
...
(	[NewAssessmentId] [bigint] IDENTITY(400,1) NOT NULL,
...
select @ToClientId=30, @FromClientId=471311013
...
AND a.AcctNum IN (SELECT [AcctNum] FROM [DataFixes]..[SeglerMove])
...
AND AcctNum IN  (SELECT [AcctNum] FROM [DataFixes]..[SeglerMove])   
...
tempdb..tempbpplocations t where l.ClientId = @FromClientId and l.LocationId = t.OldLocationId
...
tempdb..temprelocations t where l.ClientId = @FromClientId and l.LocationId = t.OldLocationId
...
AND a.AssessmentId = ta.OldAssessmentId
...
and tmp.OldAssessmentId = a.AssessmentId
AND a.AcctNum IN  (SELECT [AcctNum] FROM [DataFixes]..[SeglerMove])   
