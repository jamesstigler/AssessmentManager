...
Public Class clsData
    ...
    Public Function StoreStateFormBLOB(ByVal sFile As String, ByVal lFormId As Long, ByVal iTaxYear As Integer,
        ...
        sSQL = "DELETE StateFormsXRef FROM StateFormsXRef x, StateForms s" &
            " WHERE x.FormId = s.FormId AND x.TaxYear = s.TaxYear" &
            " AND s.FormName = " & QuoStr(sFormName) & " And s.StateCd = " & QuoStr(sStateCd) &
            " AND s.TaxYear = " & iTaxYear
        ExecuteSQL(sSQL)
        sSQL = "DELETE StateForms WHERE FormName = " & QuoStr(sFormName) & " AND StateCd = " & QuoStr(sStateCd) &
            " AND TaxYear = " & iTaxYear
        ExecuteSQL(sSQL)

        fs.Read(BLOB, 0, bloblen)
        fs.Close()
        sSQL = "INSERT StateForms (FormId, TaxYear,StateCd,FormName,FormDescription,FormData,AddUser)" &
    ...
    Public Function StoreClientFormBLOB(ByVal sFile As String, ByVal lFormId As Long, ByVal lClientId As Long,
            ByVal iTaxYear As Integer,
            ByVal sStateCd As String, ByVal sFormName As String, ByVal sFormDescription As String, ByVal sUserId As String) As Boolean
        ...
        sSQL = "DELETE ClientFormsXRef FROM ClientFormsXRef x, ClientForms s" &
            " WHERE x.FormId = s.FormId AND x.TaxYear = s.TaxYear" &
            " AND s.FormName = " & QuoStr(sFormName) & " And s.StateCd = " & QuoStr(sStateCd) &
            " AND s.TaxYear = " & iTaxYear & " AND s.ClientId = " & lClientId
        ExecuteSQL(sSQL)
        sSQL = "DELETE ClientForms WHERE FormName = " & QuoStr(sFormName) & " AND StateCd = " & QuoStr(sStateCd) &
            " AND TaxYear = " & iTaxYear & " AND ClientId = " & lClientId
        ExecuteSQL(sSQL)

        fs.Read(BLOB, 0, bloblen)
        fs.Close()
        sSQL = "INSERT ClientForms (FormId,ClientId,TaxYear,StateCd,FormName,FormDescription,FormData,AddUser)" &
    ...
    Public Function StoreTaxBillBLOB(ByVal sTable As String, ByVal lClientId As Long, ByVal lLocationId As Long,
            ByVal lAssessmentId As Long, ByVal lCollectorId As Long, ByVal iTaxYear As Integer, ByVal sUserName As String,
    ...
    Public Function RetrieveTaxBillBLOB(ByVal sTable As String, ByVal lClientId As Long, ByVal lLocationId As Long, ByVal lAssessmentId As Long,
            ByVal lCollectorId As Long, ByVal iTaxYear As Integer, ByVal sFileName As String) As Boolean
        'Try
        Dim cmd As New SqlCommand _
           ("SELECT FormData FROM " & sTable & " WHERE ClientId = @clientid" &
