DROP PROCEDURE [dbo].[spUpdBPPTaxBillDownload] 
GO

CREATE PROCEDURE [dbo].[spUpdBPPTaxBillDownload]
	@TaxYear int,
...
BEGIN	
	...
	UPDATE AssessmentDetailBPP set TaxBillDownloadDate = GETDATE(), TaxBillDownloadUser = @User

	FROM (
		SELECT 'BPP' AS PropertyType, TaxYear, Clients_Name, ClientId, LocationId, Locations_Address, Locations_ClientLocationId, 
		...
		FROM (SELECT l.TaxYear, c.Name AS Clients_Name, c.ClientId, l.LocationId, l.Address AS Locations_Address, 
		...
		GROUP BY TaxYear, Clients_Name, ClientId, LocationId, Locations_Address, Locations_ClientLocationId, Locations_Name, 
		...
		INNER JOIN AssessmentDetailBPP ad2 on 
			ad2.ClientId=tempbills.ClientId and ad2.LocationId=tempbills.LocationId and tempbills.AssessmentId=ad2.AssessmentId
			inner join Collectors collect2 on collect2.CollectorId=tempbills.CollectorId and collect2.TaxYear=tempbills.TaxYear
			and ad2.TaxYear = tempbills.TaxYear
		WHERE ad2.TaxBillDownloadDate is null and ad2.taxbillprinteddate is not null
