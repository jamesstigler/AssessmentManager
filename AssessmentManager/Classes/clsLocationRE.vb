Public Class clsLocationRE
    ...
    Public Function CreateLocation(ByVal bHitDB As Boolean, ByVal lLocationId As Long, ByRef sError As String) As Boolean
        ...
        Try
            ...
            End If
            sError = ""
            sSQL = "SELECT AddDate, AddUser FROM LocationsRE WHERE ClientId = " & m_ClientId & " AND Address = " & QuoStr(m_Address) & " AND City = " & QuoStr(m_City) & " AND StateCd = " & QuoStr(m_StateCd) & " AND TaxYear = " & m_TaxYear
            ...
            Else
                If bHitDB Then
                    If lLocationId = 0 Then lLocationId = CreateID(enumTable.enumLocationRE)
                    sSQL = "create table #tmpyears (TaxYear smallint)" &
                        " INSERT #tmpyears (TaxYear) SELECT DISTINCT TaxYear FROM LocationsRE" &
                        " INSERT LocationsRE (ClientId,LocationId,TaxYear,Address,City,StateCd,Name,Zip,LegalDescription," &
