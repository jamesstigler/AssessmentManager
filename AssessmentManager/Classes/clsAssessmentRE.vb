Public Class clsAssessmentRE
    ...
    Private m_Location As clsLocationRE
    Private m_Assets As Collection
    Private m_Comment As String

    Public Property Location() As clsLocationRE
        Get
            Return m_Location
        End Get
        Set(ByVal value As clsLocationRE)
            m_Location = value
        End Set
    End Property
    ...
    End Function
    Public Function CreateAssessment(ByVal bHitDB As Boolean, ByVal lAssessmentId As Long, ByVal lAssessorId As Long, ByRef sError As String) As Boolean
        Dim sSQL As String, dt As New DataTable
        ...
        Try
            ...
            sError = ""
            If bHitDB Then
                If lAssessmentId = 0 Then lAssessmentId = CreateID(enumTable.enumAssessmentRE)
                sSQL = "create table #tmpyears (TaxYear smallint)" &
                    " INSERT #tmpyears (TaxYear) SELECT DISTINCT TaxYear FROM LocationsRE" &
                    " WHERE ClientId = " & m_ClientId & " AND LocationId = " & m_LocationId &
                    " INSERT AssessmentsRE (ClientId,LocationId,AssessmentId,TaxYear,AcctNum,Comment,InactiveFl,AssessorId,AddUser) SELECT " &
